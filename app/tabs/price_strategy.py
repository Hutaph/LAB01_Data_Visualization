...
<div class="sec" style="margin-top:0">BÀI TOÁN & MỤC TIÊU PHÂN TÍCH</div>
<div class="cc" style="margin-bottom: 22px; border-left: 3px solid var(--pr); padding: 16px 20px;">
    <div style="font-size: 13px; color: var(--t1); line-height: 1.6;">
        <div style="margin-bottom: 10px;"><strong style="color: var(--dk);">Bài toán chung:</strong> Tối ưu hóa hiệu quả bán hàng trên sàn thương mại điện tử thông qua việc thấu hiểu hành vi khách hàng và các yếu tố thúc đẩy doanh số.</div>
        <div style="margin-bottom: 10px;"><strong style="color: var(--dk);">Hướng phân tích:</strong> Phân tích chiến lược định giá (Pricing Strategy) và mức độ nhạy cảm về giá của người tiêu dùng.</div>
        <div style="margin-bottom: 6px;"><strong style="color: var(--dk);">Mục tiêu phân tích (SMART):</strong></div>
        <ul style="padding-left: 20px; color: var(--t2);">
            <li style="margin-bottom: 6px;"><b>Mục tiêu 1:</b> Phân tích tương quan giữa giá và doanh số trên 3 phân khúc (Bình dân, Trung cấp, Cao cấp) để xác định "vùng giá tối ưu", hỗ trợ định giá sản phẩm mới nhằm <b>tăng 10% doanh thu trung bình</b> trong <b>Quý 3/2026</b>.</li>
            <li><b>Mục tiêu 2:</b> Đánh giá tác động của tỷ lệ giảm giá (Discount) đến lượng sản phẩm bán ra để đề xuất chiến lược khuyến mãi, kỳ vọng thúc đẩy <b>tăng trưởng 5% doanh số (Sales Volume)</b> trong <b>các đợt Mega Sale cuối năm nay</b>.</li>
        </ul>
    </div>
</div>
...
def render(df):
    ...
    p=_payload(d)
    components.html(_HTML.replace("__PAYLOAD__",json.dumps(p,ensure_ascii=False)),height=2400,scrolling=False)