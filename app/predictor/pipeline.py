"""
Preprocessing pipeline builder for predictions
--------------------------------------------

This module implements a sklearn-friendly preprocessing pipeline that
mirrors the notebook's data-preprocessing steps (placeholders -> NaN,
type coercion, log-transform + clipping, discount features, one-hot
encoding for categories, standard scaling).

Expose:
- `build_processor(sample_df)` -> fitted Pipeline
- `load_processor(path)`, `save_processor(proc, path)`
- `ensure_processor(path, sample_df)` -> load or build+save
- `transform_with_feature_names(processor, X)` -> DataFrame

Keep the implementation self-contained so the web app can build a
processor from a representative CSV if a saved joblib is missing.
"""
